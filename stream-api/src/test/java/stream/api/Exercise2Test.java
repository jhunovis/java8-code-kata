...
public class Exercise2Test extends ClassicOnlineStore {
    ...
    public void descSortByAge() {
         ...
         */
        Comparator<Integer> descOrder = (first, second) -> first.equals(second) ? 0 : first < second ? 1 : -1;
    ...
    public void top3RichCustomer() {
        ...
        Stream<String> top3RichCustomerStream = customerList.stream()
                .sorted((first, second) -> {
                    Integer firstBudget = first.getBudget();
                    Integer secondBudget = second.getBudget();
                    return firstBudget.equals(secondBudget) ? 0
                            : firstBudget < secondBudget ? 1 : -1; })
                .limit(3)
