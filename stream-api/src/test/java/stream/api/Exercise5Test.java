...
public class Exercise5Test extends ClassicOnlineStore {
    ...
    public void nameList() {
         ...
         */
        List<String> nameList = customerList.stream().map(c -> c.getName()).collect(Collectors.toList());
    ...
    public void ageSet() {
         ...
         */
        Set<Integer> ageSet = customerList.stream().map(c -> c.getAge()).collect(Collectors.toSet());
    ...
    public void nameInCsv() {
         ...
         */
        String string = customerList.stream().map(c -> c.getName()).collect(Collectors.joining(",", "[", "]"));
    ...
    public void oldestCustomer() {
         ...
         */
        Optional<Customer> oldestCustomer = customerList.stream().collect(Collectors.maxBy(
                (c1, c2) -> {
                    Integer c1Age = c1.getAge();
                    Integer c2Age = c2.getAge();
                    return c1Age.equals(c2Age) ? 0 : c1Age < c2Age ? -1 : 1;
                }
        ));
